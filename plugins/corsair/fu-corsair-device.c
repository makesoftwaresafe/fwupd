...
fu_corsair_device_probe(FuDevice *device, GError **error)
{
	...
	if (self->cmd_write_size > FU_CORSAIR_MAX_CMD_SIZE ||
	    self->cmd_read_size > FU_CORSAIR_MAX_CMD_SIZE) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
	...
	self->bp = fu_corsair_bp_new(usb_device, FALSE, self->epin, self->epout);
	fu_corsair_bp_set_cmd_size(self->bp, self->cmd_write_size, self->cmd_read_size);
...
fu_corsair_poll_subdevice(FuDevice *device, gboolean *subdevice_added, GError **error)
{
	...
	child_bp = fu_corsair_bp_clone(self->bp);
	fu_corsair_bp_set_is_subdevice(child_bp, TRUE);
	child = fu_corsair_device_new(self, child_bp);

	fu_device_add_instance_id(FU_DEVICE(child), self->subdeviceId);
...
static gboolean
fu_corsair_device_set_mode(FuDevice *device, FuCorsairDeviceMode mode, GError **error)
{
	FuCorsairDevice *self = FU_CORSAIR_DEVICE(device);
	gboolean is_legacy_attach =
	    fu_device_has_private_flag(device, FU_CORSAIR_DEVICE_FLAG_LEGACY_ATTACH) &&
	    mode == FU_CORSAIR_DEVICE_MODE_APPLICATION;

	if (is_legacy_attach) {
		if (!fu_corsair_bp_attach_legacy(self->bp, error)) {
			g_prefix_error(error, "legacy switch to application failed: ");
			return FALSE;
		}
	} else {
		if (!fu_corsair_bp_set_mode(self->bp, mode, error)) {
			g_prefix_error(error, "cannot set mode: ");
			return FALSE;
		}
	}
...
fu_corsair_ensure_mode(FuDevice *device, FuCorsairDeviceMode mode, GError **error)
{
	...
	if (!fu_corsair_device_set_mode(device, mode, error)) {
		g_prefix_error(error, "set mode failed: ");
		return FALSE;
	}
				 ...
				 GError **error)
{
	...
	if (!fu_corsair_bp_write_firmware(self->bp,
					  firmware,
...
fu_corsair_set_quirk_kv(FuDevice *device, const gchar *key, const gchar *value, GError **error)
{
	...
	if (g_strcmp0(key, "CorsairDeviceKind") == 0) {
		...
		self->subdeviceId = g_strdup(value);
		return TRUE;
	}
