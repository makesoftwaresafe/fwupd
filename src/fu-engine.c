...
fu_engine_load(FuEngine *self, FuEngineLoadFlags flags, FuProgress *progress, GError **error)
{
	...
	/* progress */
	fu_progress_set_id(progress, G_STRLOC);
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "read-config");
	if (flags & FU_ENGINE_LOAD_FLAG_REMOTES)
		fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "read-remotes");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "ensure-client-cert");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "write-db");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "load-plugins");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "load-quirks");
	if (flags & FU_ENGINE_LOAD_FLAG_HWINFO)
		fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "load-hwinfo");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "load-appstream");
	if (flags & FU_ENGINE_LOAD_FLAG_COLDPLUG)
		fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "backend-setup");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "plugins-init");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "hwid-quirks");
	if (flags & FU_ENGINE_LOAD_FLAG_COLDPLUG) {
		fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "plugins-setup");
		fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 3, "plugins-coldplug");
	}
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 90, "backend-coldplug");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "update-history-db");
