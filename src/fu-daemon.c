...
fu_daemon_setup(FuDaemon *self, const gchar *socket_address, GError **error)
{
	...
	fu_progress_set_profile(progress, g_getenv("FWUPD_VERBOSE") != NULL);
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 99, "load-engine");
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "load-introspection");
#ifdef HAVE_POLKIT
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "load-authority");
#endif
	fu_progress_add_step_full(progress, FWUPD_STATUS_LOADING, 1, "own-name");
